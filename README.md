...
這是一個可以部署到 Cloudflare Pages 的公司內部旅遊獎勵轉讓公告板。一般同仁不需要註冊、不需要 Email，使用匯入名單中的員工編號加統一密碼即可刊登或管理自己的公告。管理員才需要登入後台。

平台只提供公告刊登與瀏覽，不處理付款、議價、匯款、成交確認或買方登記。因為一般同仁使用的是工號加統一密碼，正式啟用時建議只把網址提供給公司內部同仁使用，不要公開散播。

## 功能

- 公開公告列表：顯示刊登中且未過期的公告，不顯示員工編號。
- 我要刊登：輸入員工編號與統一密碼後自動帶出姓名、單位、國家。
- 管理我的公告：同仁輸入員工編號與統一密碼後，可修改、標記已售出、下架、重新刊登舊公告。
...
8. 到 Settings > Variables and Secrets 新增：
   ...
   - `EMPLOYEE_SHARED_PASSWORD`：Secret，統一同仁密碼，請在 Cloudflare 裡填正式密碼
...
3. 到 `/post.html` 輸入 `ZZ999` 與正式統一密碼，應顯示查無得獎資料。
4. 到 `/post.html` 輸入 `A001` 與正式統一密碼，應帶出王小明、業務一處、日本。
5. 填寫售價、是否可議價、聯絡方式後送出。
6. 首頁應看到公告，但不會顯示員工編號。
7. 再用 `A001` 刊登一次，應提示已有刊登中的公告。
8. 到 `/my.html` 輸入 `A001` 與正式統一密碼，可修改、標記已售出或下架。
...
- 同仁統一密碼：`EMPLOYEE_SHARED_PASSWORD`
